...
function PIRApp({ user }) {
    ...
    const { handleLifeTouchStart, handleLifeTouchMove, handleLifeTouchEnd } =
        window.GLRSApp.shared.touchHandlers.createLifeTouchHandlers(
            lifeTouchStart, lifeTouchEnd, lifeCardIndex,
            setLifeTouchStart, setLifeTouchEnd, setLifeIsDragging, setLifeCardIndex
        );

    // Finances Tab Touch Handlers
    const { handleFinancesTouchStart, handleFinancesTouchMove, handleFinancesTouchEnd } =
        window.GLRSApp.shared.touchHandlers.createFinancesTouchHandlers(
            financesTouchStart, financesTouchEnd, financesCardIndex,
            setFinancesTouchStart, setFinancesTouchEnd, setFinancesIsDragging, setFinancesCardIndex
        );

    // Wellness Tab Touch Handlers
    const { handleWellnessTouchStart, handleWellnessTouchMove, handleWellnessTouchEnd } =
        window.GLRSApp.shared.touchHandlers.createWellnessTouchHandlers(
            wellnessTouchStart, wellnessTouchEnd, wellnessCardIndex,
            setWellnessTouchStart, setWellnessTouchEnd, setWellnessIsDragging, setWellnessCardIndex
        );
    ...
    const { handleRefresh, handleTouchStart, handleTouchMove, handleTouchEnd } =
    ...
    window.GLRSApp.hooks.useLucideIcons([
        currentView, showModal, showIncompleteTasksModal, showSidebar,
        showHabitTrackerModal, showQuickReflectionModal, showThisWeekTasksModal,
        showOverdueItemsModal, showMarkCompleteModal, showProgressStatsModal,
        showGoalProgressModal, showTodayWinsModal, showHabitHistory,
        showReflectionHistory, showWinsHistory, lifeCardIndex
    ]);
    ...
    window.GLRSApp.hooks.useSavingsData(
        user, db, setSavingsItems, setSavingsGoals, setMoneyMapStops,
        setActiveSavingsGoal, setActualMoneySaved, setCustomGoalItems
    );
...
const completeAssignment = async (assignmentId, goalId, reflection) => {
    await window.GLRSApp.shared.assignmentActions.completeAssignment({
        assignmentId, goalId, reflection, user, db, firebase,
        loadAssignments, loadGoals, loadComplianceRates, updateGoalProgress
    });
};
...
const sendCommunityMessage = async (message, imageUrl) => {
    await window.GLRSApp.shared.messagingActions.sendCommunityMessage({
        message, imageUrl, user, userData, db, firebase
    });
};
...
const sendTopicRoomMessage = async (roomId, content, imageFile = null) => {
    await window.GLRSApp.shared.messagingActions.sendTopicRoomMessage({
        roomId, content, imageFile, user, userData, db, firebase, setTopicRoomMessages
    });
};
...
const enterTopicRoom = async (room) => {
    await window.GLRSApp.shared.messagingActions.enterTopicRoom({
        room, db, setActiveTopicRoom, setTopicRoomMessages, setShowModal
    });
};
...
const exportDataAsJSON = () => {
    window.GLRSApp.shared.exportActions.exportDataAsJSON({
        userData, checkIns, goals, assignments, sobrietyDays, checkInStreak, complianceRate
    });
};
...
const exportDataAsPDF = () => {
    window.GLRSApp.shared.exportActions.exportDataAsPDF({
        userData, user, sobrietyDays, moneySaved, checkInStreak, complianceRate, checkIns, goals
    });
};
...
const dismissBroadcast = () => {
    window.GLRSApp.shared.uiActions.dismissBroadcast({
        setBroadcastDismissed, setActiveBroadcast
    });
};
...
return (
    <div className="app-container">
        {/* Header Bar - Extracted to /Index/components/HeaderBar.js */}
        {/* ✅ PHASE 8C-5: HeaderBar now uses Context - no props needed */}
        {React.createElement(window.GLRSApp.components.HeaderBar)}

        {/* Pull-to-Refresh Indicator - Extracted to /Index/components/PullToRefreshIndicator.js */}
        {/* ✅ PHASE 8C-5: PullToRefreshIndicator now uses Context - no props needed */}
        {React.createElement(window.GLRSApp.components.PullToRefreshIndicator)}

        {/* Main Content - Extracted to /Index/components/MainContent.js */}
        {/* ✅ PHASE 8C-1: Reduced props - MainContent now uses Context */}
        {React.createElement(window.GLRSApp.components.MainContent, {
            contentRef,
            handleTouchStart,
            handleTouchMove,
            handleTouchEnd
        ...
        {/* ✅ PHASE 8C-5: LegalFooter now uses Context - no props needed */}
        {React.createElement(window.GLRSApp.components.LegalFooter)}

        {/* ✅ PHASE 8C-5: CrisisButton now uses Context - no props needed */}
        {React.createElement(window.GLRSApp.components.CrisisButton)}

        {/* ✅ PHASE 8C-5: BottomNavigation now uses Context - no props needed */}
        {React.createElement(window.GLRSApp.components.BottomNavigation)}

      


        {/* ✅ PHASE 8C-5: ModalRenderer now uses Context - no props needed */}
        {React.createElement(window.GLRSApp.components.ModalRenderer)}
...
function PIRAppWithContext({ user }) {
    return (
