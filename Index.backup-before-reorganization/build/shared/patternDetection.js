...
const detectDayOfWeekPattern = checkInsData => {
  ...
  let lowestMoodDay = null,
    highestCravingDay = null,
    highestAnxietyDay = null;
  let lowestMoodValue = 5,
    highestCravingValue = 0,
    highestAnxietyValue = 0;
  ...
  if (lowestMoodDay !== null && weeklyAvg.mood - lowestMoodValue >= 1) {
    return {
      ...
      tips: [`Start your ${dayNames[lowestMoodDay]} with a morning routine`, `Schedule something enjoyable for ${dayNames[lowestMoodDay]} mornings`, `Connect with your accountability partner on ${dayNames[lowestMoodDay]}`, `Review your ${dayNames[lowestMoodDay]} triggers from past weeks`]
    };
  }
  if (highestCravingDay !== null && highestCravingValue - weeklyAvg.craving >= 1) {
    return {
      ...
      tips: [`Identify your ${dayNames[highestCravingDay]} triggers (stress? social plans?)`, `Plan alternative activities for ${dayNames[highestCravingDay]} evenings`, `Check in with your support group on ${dayNames[highestCravingDay]}s`, `Practice grounding techniques on ${dayNames[highestCravingDay]} afternoons`]
    };
  }
  if (highestAnxietyDay !== null && highestAnxietyValue - weeklyAvg.anxiety >= 1) {
    return {
      ...
      tips: [`Prepare for the week ahead on ${dayNames[highestAnxietyDay - 1 >= 0 ? highestAnxietyDay - 1 : 6]} night`, `Practice ${dayNames[highestAnxietyDay]} evening meditation`, `Limit social media on ${dayNames[highestAnxietyDay]}s`, `Connect with supportive friends on ${dayNames[highestAnxietyDay]}`]
    };
  }
  return null;
};
...
const detectConsecutiveDaysPattern = checkInsData => {
  ...
  if (maxConsecutiveLowMood >= 3) {
    return {
      ...
      tips: ['Consider reaching out to your coach today', 'Review your coping strategies - what\'s working?', 'Have you been completing your daily check-ins?', 'Physical activity can help - take a walk today']
    };
  }
  if (maxConsecutiveHighCraving >= 3) {
    return {
      ...
      tips: ['This is normal - cravings pass, you are strong', 'Call your accountability partner right now', 'Use the 5-minute rule: delay for 5 minutes, reassess', 'Review your relapse prevention plan in Guides']
    };
  }
  return null;
};
...
const detectTimeOfDayPattern = checkInsData => {
  ...
  if (difference <= -1) {
    return {
      ...
      tips: ['Establish a consistent morning routine', 'Avoid checking phone first thing in the morning', 'Try morning meditation or stretching', 'Plan something positive for each morning']
    };
  }
  if (difference >= 1) {
    return {
      ...
      tips: ['Avoid isolation in the evenings', 'Limit screen time after 8 PM', 'Call a friend or family member each evening', 'Practice evening reflection to process your day']
    };
  }
  return null;
};
...
const detectPatterns = checkInsData => {
  ...
  return detectDayOfWeekPattern(checkInsData) || detectConsecutiveDaysPattern(checkInsData) || detectTimeOfDayPattern(checkInsData);
};
