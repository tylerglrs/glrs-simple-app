...
const useHandlers = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await db.collection('topicRooms').doc(activeTopicRoom?.id).collection('messages').add(messageData);
      setTopicMessage('');
  ...
  const handleMorningCheckIn = async data => {
    try {
      ...
      const existingCheckIn = await db.collection('checkIns').where('userId', '==', user.uid).where('createdAt', '>=', today).where('createdAt', '<', tomorrow).get();
