...
const setupRealtimeListeners = (db, user, listenersRef, broadcastDismissed, setters, loadFunctions) => {
  ...
  const notifListener = db.collection('notifications').where('userId', '==', user.uid).orderBy('createdAt', 'desc').limit(20).onSnapshot(snapshot => {
  ...
  const messagesListener = db.collection('messages').where('roomId', '==', 'main').orderBy('createdAt', 'desc').limit(50).onSnapshot(snapshot => {
    const messages = [];
    snapshot.forEach(doc => {
      messages.push({
        id: doc.id,
  ...
  // Goals listener
  const goalsListener = db.collection('goals').where('userId', '==', user.uid).where('status', '==', 'active').onSnapshot(snapshot => {
    loadGoals();
  });
  listenersRef.current.push(goalsListener);

  // Assignments listener
  const assignmentsListener = db.collection('assignments').where('userId', '==', user.uid).onSnapshot(snapshot => {
    loadAssignments();
  });
  listenersRef.current.push(assignmentsListener);

  // Habits listener
  const habitsListener = db.collection('habits').where('userId', '==', user.uid).onSnapshot(snapshot => {
    loadHabits();
    loadTodayHabits();
  });
  ...
  const habitCompletionsListener = db.collection('habitCompletions').where('userId', '==', user.uid).onSnapshot(snapshot => {
    loadTodayHabits();
  });
  listenersRef.current.push(habitCompletionsListener);

  // Quick reflections listener
  const quickReflectionsListener = db.collection('quickReflections').where('userId', '==', user.uid).onSnapshot(snapshot => {
    loadQuickReflections();
  });
  listenersRef.current.push(quickReflectionsListener);

  // Wins listener
  const winsListener = db.collection('wins').where('userId', '==', user.uid).onSnapshot(snapshot => {
    loadTodayWins();
  });
  listenersRef.current.push(winsListener);

  // Broadcasts listener
  const broadcastsListener = db.collection('broadcasts').where('active', '==', true).orderBy('createdAt', 'desc').limit(1).onSnapshot(snapshot => {
