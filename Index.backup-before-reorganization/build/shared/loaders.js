...
const useLoaders = () => {
  ...
  const loadAllData = async () => {
    try {
      setLoading(true);
      await loadUserData();
      await Promise.all([loadTopicRooms(), loadMeetings(), loadEmergencyResources(), loadGoals(), loadAssignments(), loadDailyInspiration(), loadMilestones(), loadBroadcasts(), loadResources(), loadSupportGroups(), loadCheckIns(), loadTodaysPledge(), loadStreak(), loadCoachNotes(), loadReflections(), loadComplianceRates(), window.GLRSApp.calculations.calculateMilestones(), checkMilestoneNotifications(), loadDailyTasksStatus(), window.GLRSApp.calculations.calculateTotalCheckIns().then(count => setTotalCheckIns(count)), window.GLRSApp.calculations.calculateStreaks(), window.GLRSApp.calculations.calculateReflectionStreaks()]);
    } catch (error) {} finally {
      setLoading(false);
    }
  };
  ...
  const loadTopicRooms = async () => {
    try {
      const roomsSnap = await db.collection('topicRooms').where('active', '==', true).get();
      const roomsData = [];
  ...
  const loadMeetings = async () => {
    try {
      ...
      const assignedMeetingsSnap = await db.collection('meetings').where('assignedPIRs', 'array-contains', currentUserId).where('status', '==', 'scheduled').get();

      // Get global meetings (for all PIRs)
      const globalMeetingsSnap = await db.collection('meetings').where('isGlobal', '==', true).where('status', '==', 'scheduled').get();

      // Combine both results and remove duplicates
      const meetingsMap = new Map();
      assignedMeetingsSnap.forEach(doc => {
        meetingsMap.set(doc.id, {
          id: doc.id,
          ...doc.data()
        });
      });
  ...
  const loadSupportGroups = async () => {
    try {
      const groupsSnap = await db.collection('supportGroups').where('active', '==', true).orderBy('day').get();
      const groupsData = [];
  ...
  const loadEmergencyResources = async () => {
    try {
      const emergencySnap = await db.collection('resources').where('category', '==', 'emergency').where('active', '==', true).get();
      const emergencyData = [];
  ...
  const loadGoals = async () => {
    try {
      ...
      const goalsSnap = await db.collection('goals').where('userId', '==', user.uid).orderBy('createdAt', 'desc').get();
      const goalsData = [];
      for (const doc of goalsSnap.docs) {
        ...
        const assignmentsSnap = await db.collection('assignments').where('goalId', '==', doc.id).where('userId', '==', user.uid).get();
        const assignments = [];
        ...
        goalData.progress = assignments.length > 0 ? Math.round(completed / assignments.length * 100) : 0;
  ...
  const loadAssignments = async () => {
    try {
      const assignmentsSnap = await db.collection('assignments').where('userId', '==', user.uid).orderBy('createdAt', 'desc').get();
  ...
  const loadHabits = async () => {
    try {
      const habitsSnap = await db.collection('habits').where('userId', '==', user.uid).orderBy('createdAt', 'desc').get();
      const habitsData = [];
  ...
  const loadTodayHabits = async () => {
    try {
      ...
      const userNow = new Date(new Date().toLocaleString("en-US", {
      ...
      const todayHabitsSnap = await db.collection('habitCompletions').where('userId', '==', user.uid).where('completedAt', '>=', firebase.firestore.Timestamp.fromDate(today)).where('completedAt', '<', firebase.firestore.Timestamp.fromDate(tomorrow)).get();
  ...
  const loadQuickReflections = async () => {
    try {
      const reflectionsSnap = await db.collection('quickReflections').where('userId', '==', user.uid).orderBy('createdAt', 'desc').limit(50).get();
  ...
  const loadTodayWins = async () => {
    try {
      ...
      const tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);
      const winsSnap = await db.collection('wins').where('userId', '==', user.uid).where('createdAt', '>=', firebase.firestore.Timestamp.fromDate(today)).where('createdAt', '<', firebase.firestore.Timestamp.fromDate(tomorrow)).orderBy('createdAt', 'desc').get();
      const winsData = [];
  ...
  const loadDailyTasksStatus = async () => {
    try {
      ...
      const todayCheckInsSnap = await db.collection('checkIns').where('userId', '==', user.uid).where('createdAt', '>=', today).where('createdAt', '<', tomorrow).get();
      ...
      const assignmentsSnap = await db.collection('assignments').where('userId', '==', user.uid).get();
      ...
      return {
        ...
        // Return only incomplete for display
  ...
  const flagContent = async (contentType, contentData) => {
    ...
    try {
      await db.collection('flaggedContent').add({
        ...
        // 'topic_message' or 'community_message'
        ...contentData,
        flaggedBy: user.uid,
  ...
  const checkMilestoneNotifications = async () => {
    ...
    for (const milestone of recoveryMilestones) {
      ...
      const existingNotif = await db.collection('notifications').where('recipientId', '==', user.uid).where('type', 'in', ['milestone_today', 'milestone_tomorrow']).where('milestoneTitle', '==', milestone.title).where('createdAt', '>=', today).limit(1).get();
  ...
  const loadDailyInspiration = async () => {
    try {
      const inspirationsSnap = await db.collection('dailyInspirations').where('active', '==', true).get();
  ...
  const loadMilestones = async () => {
    try {
      const milestonesSnap = await db.collection('milestones').orderBy('daysRequired', 'asc').get();
  ...
  const loadBroadcasts = async () => {
    try {
      const broadcastsSnap = await db.collection('broadcasts').where('active', '==', true).orderBy('createdAt', 'desc').limit(1).get();
  ...
  const loadResources = async () => {
    try {
      ...
      const resourcesSnap = await db.collection('resources').where('active', '==', true).get();
      const resourcesData = {
        videos: [],
        articles: [],
        tools: [],
      ...
      resourcesSnap.forEach(doc => {
        ...
        const shouldShowResource = resource.isGlobal === true ||
        // Global resource
        resource.assignedPIRs && resource.assignedPIRs.includes(user.uid) ||
  ...
  const loadCheckIns = async () => {
    try {
      const checkInsSnapshot = await db.collection('checkIns').where('userId', '==', user.uid).orderBy('createdAt', 'desc').limit(30).get();
      const checkInsList = [];
  ...
  const loadStreakCheckIns = async () => {
    try {
      ...
      const checkInsSnapshot = await db.collection('checkIns').where('userId', '==', user.uid).orderBy('createdAt', 'desc').limit(90).get();
      const allCheckIns = [];
      checkInsSnapshot.forEach(doc => {
        allCheckIns.push({
          id: doc.id,
      ...
      while (consecutiveDays && dayIndex < 365) {
        ...
        const dayCheckIn = allCheckIns.find(checkIn => {
          ...
          const checkInUserTZ = new Date(checkInDate.toLocaleString("en-US", {
            timeZone: userTimezone
          }));
  ...
  const loadTodaysPledge = async () => {
    try {
      ...
      const tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);
      const pledgeSnapshot = await db.collection('pledges').where('userId', '==', user.uid).where('createdAt', '>=', today).where('createdAt', '<', tomorrow).get();
  ...
  const loadStreak = async () => {
    try {
      ...
      const checkInsSnapshot = await db.collection('checkIns').where('userId', '==', user.uid).where('createdAt', '>=', ninetyDaysAgo).orderBy('createdAt', 'desc').get();
      const checkInsList = [];
      checkInsSnapshot.forEach(doc => {
        checkInsList.push({
          id: doc.id,
      ...
      const moodValues = last30DaysCheckIns.filter(c => c.morningData?.mood).map(c => c.morningData.mood);
      const avgMood = moodValues.length > 0 ? (moodValues.reduce((a, b) => a + b, 0) / moodValues.length).toFixed(1) : 0;
  ...
  const loadCoachNotes = async () => {
    try {
      const notesSnapshot = await db.collection('coachNotes').where('userId', '==', user.uid).orderBy('createdAt', 'desc').limit(5).get();
      const notesList = [];
  ...
  const loadCalendarHeatmapData = async () => {
    try {
      ...
      const snapshot = await db.collection('checkIns').where('userId', '==', user.uid).where('createdAt', '>=', oneYearAgo).orderBy('createdAt', 'desc').get();
      ...
      const calendarArray = Object.values(calendarData).sort((a, b) => new Date(a.dateKey) - new Date(b.dateKey));
  ...
  const loadMoodWeekData = async () => {
    try {
      ...
      const snapshot = await db.collection('checkIns').where('userId', '==', user.uid).orderBy('createdAt', 'desc').get();
      ...
      const moodArray = Object.values(moodByDay).sort((a, b) => new Date(a.dateKey) - new Date(b.dateKey));
      ...
      const thisWeekAvg = thisWeekMoods.length > 0 ? thisWeekMoods.reduce((sum, m) => sum + m.mood, 0) / thisWeekMoods.length : 0;
      const lastWeekAvg = lastWeekMoods.length > 0 ? lastWeekMoods.reduce((sum, m) => sum + m.mood, 0) / lastWeekMoods.length : 0;
  ...
  const loadOverallDayWeekData = async () => {
    try {
      ...
      // Convert to array and sort by date (oldest first)
      const overallDayArray = Object.values(overallDayByDay).sort((a, b) => new Date(a.dateKey) - new Date(b.dateKey));
      ...
      // Calculate averages
      const thisWeekAvg = thisWeekScores.length > 0 ? thisWeekScores.reduce((sum, m) => sum + m.overallDay, 0) / thisWeekScores.length : 0;
      const lastWeekAvg = lastWeekScores.length > 0 ? lastWeekScores.reduce((sum, m) => sum + m.overallDay, 0) / lastWeekScores.length : 0;
      ...
      const datesWithReflections = Object.keys(overallDayByDay).sort((a, b) => new Date(b) - new Date(a)).slice(0, 7); // Take the 7 most recent days WITH reflections
  ...
  const loadGratitudeInsights = async () => {
    try {
      ...
      const insightsRef = db.collection('users').doc(user.uid).collection('insights').doc('gratitude');
  ...
  const loadDailyQuotes = async () => {
    try {
      const quotesSnapshot = await db.collection('dailyQuotes').orderBy('order', 'asc').get();
  ...
  const loadGoalAchievementData = async () => {
    try {
      ...
      const yesterdayGoalSnap = await db.collection('checkIns').where('userId', '==', user.uid).where('createdAt', '>=', yesterday).where('createdAt', '<=', yesterdayEnd).get();
      ...
      const historySnap = await db.collection('goals_tracking').where('userId', '==', user.uid).orderBy('checkedInAt', 'desc').limit(30).get();
  ...
  const shareReflections = async () => {
    try {
      ...
      const snapshot = await db.collection('checkIns').where('userId', '==', user.uid).where('createdAt', '>=', thirtyDaysAgo).orderBy('createdAt', 'desc').get();
      const reflections = [];
      snapshot.docs.forEach(doc => {
        const checkIn = doc.data();
      ...
      // Calculate stats
      const avgScore = reflections.filter(r => r.overallDay).reduce((sum, r) => sum + r.overallDay, 0) / reflections.filter(r => r.overallDay).length;
      const gratitudeCount = reflections.filter(r => r.gratitude && r.gratitude.trim()).length;

      // Get recent gratitudes (last 5)
      const recentGratitudes = reflections.filter(r => r.gratitude && r.gratitude.trim()).slice(0, 5);
...
Shared from GLRS Lighthouse Recovery App`;
  ...
  const loadReflections = async () => {
    try {
      ...
      const checkInsSnapshot = await db.collection('checkIns').where('userId', '==', user.uid).orderBy('createdAt', 'desc').get();
      ...
      const scores = reflectionsList.filter(r => r.overallDay !== undefined && r.overallDay !== null).map(r => r.overallDay);
      const avgDailyScore = scores.length > 0 ? parseFloat((scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1)) : 0;
  ...
  const loadChallengesInsights = async () => {
    try {
      const insightsRef = db.collection('users').doc(user.uid).collection('insights').doc('challenges');
      const insightsDoc = await insightsRef.get();
      if (insightsDoc.exists) {
        const insightsData = insightsDoc.data();
  ...
  const updateStreak = async () => {
    try {
      ...
      if (streakDoc.exists) {
        ...
        const lastCheckIn = data.lastCheckIn?.toDate ? data.lastCheckIn.toDate() : new Date(data.lastCheckIn);
        const today = new Date();
