...
window.GLRSApp.services.firestore = {
  ...
  getCheckIns: async (userId, limit = 30) => {
    try {
      const snapshot = await window.GLRSApp.db.collection('checkIns').where('userId', '==', userId).orderBy('createdAt', 'desc').limit(limit).get();
  ...
  getNotifications: async (userId, limit = 20) => {
    try {
      const snapshot = await window.GLRSApp.db.collection('notifications').where('userId', '==', userId).orderBy('createdAt', 'desc').limit(limit).get();
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  getAssignments: async userId => {
    try {
      const snapshot = await window.GLRSApp.db.collection('assignments').where('userId', '==', userId).orderBy('dueDate', 'asc').get();
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  getGoals: async userId => {
    try {
      const snapshot = await window.GLRSApp.db.collection('goals').where('userId', '==', userId).orderBy('createdAt', 'desc').get();
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  getMessages: async (userId, limit = 50) => {
    try {
      const snapshot = await window.GLRSApp.db.collection('messages').where('participants', 'array-contains', userId).orderBy('createdAt', 'desc').limit(limit).get();
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  getHabits: async userId => {
    try {
      const snapshot = await window.GLRSApp.db.collection('habits').where('userId', '==', userId).orderBy('createdAt', 'desc').get();
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
  ...
  listenToNotifications: (userId, callback) => {
    return window.GLRSApp.db.collection('notifications').where('userId', '==', userId).orderBy('createdAt', 'desc').limit(20).onSnapshot(snapshot => {
  ...
  listenToCheckIns: (userId, callback) => {
    return window.GLRSApp.db.collection('checkIns').where('userId', '==', userId).orderBy('createdAt', 'desc').limit(30).onSnapshot(snapshot => {
