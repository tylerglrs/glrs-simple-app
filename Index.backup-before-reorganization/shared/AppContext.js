...
const { createContext, useContext, useState, useRef } = React;
...
const AppProvider = ({ children, user }) => {
    ...
    const updateGoalProgress = async (goalId) => {
        if (window.GLRSApp?.actions?.updateGoalProgress) {
            return window.GLRSApp.actions.updateGoalProgress({ goalId, user, db, firebase });
        }
    };
...
// Register globally
window.GLRSApp = window.GLRSApp || { components: {}, hooks: {} };
